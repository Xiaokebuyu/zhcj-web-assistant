...
import top.mole9630.ada.dto.PostWithReplyCountDto;
...
public class ForumController {
    ...
    public Result<Page<PostWithReplyCountDto>> getPostList(
        ...
        List<PostWithReplyCountDto> dtoList = postPage.getRecords().stream().map(post -> {
            ...
            return new PostWithReplyCountDto(post, replyCount);
        }).collect(Collectors.toList());
        Page<PostWithReplyCountDto> resultPage = new Page<>(postPage.getCurrent(), postPage.getSize(), postPage.getTotal());
        resultPage.setRecords(dtoList);
        return Result.success(resultPage, "查询成功");
    }
//        // 获取分页数据
//        Page<Post> pageResult = postService.page(new Page<>(page, pageSize), queryWrapper);
//        // 为每个帖子设置评论总数
//        pageResult.getRecords().forEach(post -> {
//            int count = replyService.count(new LambdaQueryWrapper<Reply>()
//                    .eq(Reply::getRpFid, post.getFId()));
//            post.setFCommentCount(count); // 修改这里
//        });
//
//        System.out.println("总记录数: " + pageResult.getTotal());
//        System.out.println("总记录数: " + pageResult.getRecords());
//        return Result.success(pageResult, "查询成功");
    ...
    public Result<Page<Reply>> getReplies(
        ...
        Page<Reply> pageResult = replyService.page(new Page<>(page, pageSize), queryWrapper);
        return Result.success(pageResult, "查询成功");
    }
