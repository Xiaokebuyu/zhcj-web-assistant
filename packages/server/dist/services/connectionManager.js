...
class ConnectionManager {
     ...
     * 添加新连接
     */
    static addConnection(socketId, info) {
     ...
     * 更新连接活动时间
     */
    static updateActivity(socketId) {
        const connection = this.connections.get(socketId);
        if (connection) {
     ...
     * 清理不活跃的连接
     */
    static cleanupInactiveConnections(maxInactiveTime = 30 * 60 * 1000) {
        const now = Date.now();
        let removedCount = 0;
        for (const [socketId, connection] of this.connections) {
     ...
     * 记录连接信息
     */
    static logConnectionInfo(socketId, info) {
        console.log(`👤 新用户连接详情:`);
     ...
     * 定期打印统计信息
     */
    static startStatsLogging(intervalMinutes = 10) {
        setInterval(() => {
