...
public class GoodsController {
    ...
    @ApiOperation(value = "获取兑换记录", notes = "获取当前用户的商品兑换记录，按时间倒序排列")
    public Result<List<Exchange>> getExchangeList() {
        try {
            ...
            if (user == null || user.getUType() != 1) {
                return Result.error(0, "仅志愿者可查看兑换记录");
            }

            QueryWrapper<Exchange> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("ex_uid", userId)
                    .orderByDesc("ex_time");
            List<Exchange> records = exchangeService.list(queryWrapper);
    ...
    public Result<String> exchangeGood(@PathVariable Integer goodId) {
        try {
            ...
            // 更新商品库存
            good.setSGoodStock(good.getSGoodStock() - 1);
            scoreStoreService.updateById(good);
